...
const handleDelete = async (id) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:5000/api/products/${editProduct}`, {
        method: "PUT",
        headers: {
        ...
        <tbody>
          {products.map((product) => (
            <tr key={product._id}>
              ...
              <td>{product.website ? <a href={product.website}>Website</a> : "N/A"}</td>
              <td>
                {product.image ? <img src={product.image} alt={product.name} width="50" height="50" /> : "No Image"}
              </td>
              <td>
  <button onClick={() => handleEdit(product)}>Edit</button>
  <button onClick={() => handleDelete(product._id)} style={{ marginLeft: "5px", color: "red" }}>
    Delete
  </button>
</td>
            </tr>
